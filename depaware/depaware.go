...
func process(pkg string) {
	...
	daContents, daErr := os.ReadFile(daFile)
	...
	fmt.Fprintf(&buf, "%s dependencies: (generated by github.com/ratrocket/depaware)\n\n", pkg)
	...
	if *update {
		if err := os.WriteFile(daFile, buf.Bytes(), 0644); err != nil {
			log.Fatal(err)
		}
		return
	}

	// It would be cool to have an *option* to write to stdout, but
	// the documented interface (the usage message) says that output
	// will be written to the file given by "-file".  So let's make
	// it so.
